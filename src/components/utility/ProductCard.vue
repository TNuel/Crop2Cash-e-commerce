<template>
  <div class="max-w-md mx-auto bg-white rounded overflow-hidden">
    <div class="w-full min-h-64 object-cover rounded-xl relative">
      <img @click="gotoProductDetailsPage(productDetails.id)" :src="productDetails.image" :alt="productDetails.title" class="rounded-xl"/>
    ...
    <div class="py-4 text-textPrimary">
      <div class="font-bold font-urbanist text-2xl mb-2">
        {{ productDetails.title }}
      </div>
      <p class="text-sm lg:text-base font-bold font-urbanist">
        {{
        ...
        <span class="text-textSecondary/50 text-xs lg:text-sm line-through">{{
          new Intl.NumberFormat("en-NG", {
            style: "currency",
            currency: "NGN",
          }).format(productDetails.price / 100) ?? 0
    ...
    <div class="my-2 flex space-x-1">
      ...
      <div>
        <svg
          ...
          :class="{ 'text-textPrimary fill-current': isStarFilled }"
          @click="toggleStar"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
    ...
    <div
      class="w-full px-6 py-1 lg:py-3 flex justify-center items-center group rounded-md hover:bg-secondary/50 hover:text-white hover:scale-105 border-2 border-secondary/50 text"
    >
...
const prop = defineProps({
  productDetails: {
    type: Object,
    default(rawProps) {
      return { image: "", title: "", description: "", price: 0, id: '65q37jdct73' };
    },
  },
});
...
const addToCart = () => {
  ...
  console.log("Added to cart:", prop.productDetails.title);
  console.log("Added to cart:", prop.productDetails.image);
};
</script>
